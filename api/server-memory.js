...
console.log('📝 For production, deploy to Render with PostgreSQL');
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({ 
      status: 'OK', 
      message: 'Restaurant API is running (IN-MEMORY)',
      database: 'In-Memory (temporary)',
      timestamp: new Date().toISOString(),
      warning: 'Data will be lost when server restarts. Deploy to Render for production.'
    });
  } catch (error) {
    res.status(500).json({ 
      status: 'ERROR', 
...
app.listen(PORT, () => {
  ...
  console.log(`   - Data is temporary and will be lost on restart`);
  console.log(`   - Perfect for testing cross-device functionality`);
  console.log(`   - For production, deploy to Render with PostgreSQL\n`);
});
